...
| 1 DRY-RUN | `EVIDENCE_SETTLEMENT_ENABLED=true` + `EVIDENCE_SETTLEMENT_MODE=dry_run` | Full pipeline nightly: real snapshots read, every validation written to the dry-run directory (a split store that **refuses** snapshot appends and **cannot** address the real record — construction throws if the directories collide). Public pages byte-identical. **In production set `EVIDENCE_SETTLEMENT_DRYRUN_DIR=/opt/rankwagers/shared/evidence-archive-dryrun`** — the default is release-local (`data/evidence-archive-dryrun`) and would not survive a redeploy, and the 5-day audit window wants one continuous store beside the real one, which lives at `/opt/rankwagers/shared/evidence-archive/`. | Remove either env var. Delete the dry-run directory freely — it is not a record. |
