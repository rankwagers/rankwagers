...
import { exportCalibrationSection } from "@/lib/calibration-intelligence/service";
...
export async function GET(req: NextRequest) {
  ...
  const rl = checkRateLimitSafe({
    key: `admin-calibration-export:${clientKey({ headers: req.headers })}`,
    limit: 20,
    windowMs: 60_000,
    route: "admin_calibration_export",
  ...
  const section = parseCalibrationSection(
  ...
  trackAdminCalibrationAnalytics("admin_calibration_exported", {
    section,
    format: formatRaw,
  });

  try {
    const file = await exportCalibrationSection(section, formatRaw, filters);
